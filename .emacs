...
;; installed by default in 25.2.2(or > 24.x.x).
;;
;; (use-package pkgName
;;  :ensure t
...
;; installed.  It does this on demand, ie loads package only when you
;; want to use it for the first time, instead of on startup which
;; makes things slow
;;
...
(require 'package)
;; Add ELPA (GNU) to the list of repositories
(add-to-list 'package-archives
...
;; Custom face colors for user smagri. This should only be in one place
;; in .emacs, not  in individual hooks.  In individual  hooks in emacs
;; 28+ won't work as expected.  in one place in .emacs.

;; orig individual hook setup
;; (custom-set-faces
...
;;   "Set up desired indent style"
   ...
   (c-set-offset 'statement-case-intro 4 t)
   (c-set-offset 'case-label 4 t)
...
;; needed to remove for snap emacs version 30
;;(require 'all)
...
;; python emacs30.1 with correct faces
;; (add-hook 'python-mode-hook
;;           (lambda ()
;;             ;; Optional: clear mode name
;;             (setq mode-name "")))

(defun simones-python-mode-hook ()
  "Custom Python mode settings."
  (setq indent-tabs-mode nil)       ;; never use tabs
  (setq tab-width 4)                ;; tab width is 4
...
;; Install clangd also.
;; Activate language server support for c++, such things as std::this_thread
;; This causes the following error while loading .emacs:
;;Error (use-package): Failed to install lsp-mode: Package ‘emacs-28.1’ is unavailable
;;Error (use-package): Failed to install lsp-ui: Package ‘emacs-28.1’ is unavailable
;;Warning (initialization): An error occurred while loading ‘/home/smagri/.emacs’:

;; I tried  to enable this mode  but it seems a  little unreliable and
;; seems to disable yasnippet's. Here is the config file for a working lsp-mode
;; .emacs.lsp-mode.messesWith.yasnippit

;; starts these packages each time emacs starts
(require 'use-package)

;; Install clangd also to activate this, and see ~/.clangd.
;; Activate language server support for c++, such things as std::this_thread
;; This package is now available for emacs 30.x, not for previous versions.
;; Now you will have:
...
;; I have found this to be  unreliable at this stage, even though it's
;; great cause  it gives  me std::this_thread....   Also, it  seems to
;; disable most yasnippet's,  not desirable.  Maybe work  on this with
;; newer versions of emacs and lsp-mode.

;; It also allows you to  disable some unwarrented Flycheck errors, as
;; and example below and in ~/.flycheck and ~/.config/.flycheck.
...
;; ediff setup
;;
;; (synchronized scrolling does not work too well, can use meld
;; insted, see below)
;;
;;
;; automatic ediff vertical
...
;; To always enable linked scrolling when ediff starts, add this to your config
;; no works(add-hook 'ediff-after-setup-windows-hook 'my-ediff-sync-scroll-keys)

;;(add-hook 'ediff-startup-hook 'ediff-toggle-synchronize)
(setq ediff-scroll-horizontally t)

;; Use horizontal split (side-by-side) for Ediff
(setq ediff-split-window-function 'split-window-horizontally)
(setq ediff-window-setup-function 'ediff-setup-windows-plain)

;; Optional: disable auto recentering and allow smooth scrolling
(setq ediff-scroll-off t)
(setq ediff-even-scroll t)

;; Manual sync scrolling hook (fallback if built-in toggle is missing)
(defun my-ediff-sync-scroll ()
  "Manually synchronize scrolling in ediff buffers."
  (let ((winA (get-buffer-window ediff-buffer-A))
        (winB (get-buffer-window ediff-buffer-B)))
    (when (and winA winB)
      (let ((scroll (window-start winA)))
        (set-window-start winB scroll)))))

(defun my-ediff-startup-sync ()
  "Set up scroll syncing during ediff session."
  (add-hook 'post-command-hook 'my-ediff-sync-scroll nil t))

(add-hook 'ediff-startup-hook 'my-ediff-startup-sync)

;; Clean up after ediff quits
...
(defun meld-files (file1 file2)
  ...
  (interactive "fFile 1: \nfFile 2: ")
  (start-process "meld" nil "meld" file1 file2))
...
;; START:,
(use-package company ;; install 'company' package
  :diminish
...
;; ;; START: use company-ispell backend for text files
...
;;
;; irony-mode seems NO LONGER SUPPORTED so removed references to it/
;;
;; START:
;;
...
;; CONFIGURE semantic mode for all buffers.
;;

;; It provides  intelligent code  parsing and  analysis, it's  part of
;; CEDET(Collection of Emacs Development  Environment Tools) suite.  I
;; mainly use this here  to get more header files for  it to look into
;; so I  get more  command completion in  c/c++ mode,  especially from
;; include  files.  I  don't  know that  company-c-headers-path-system
;; works anymore.
;; seems like only these are detedcted:
;; ***** use: 'gcc -xc++ -E -v -' to detrmine your system header files *****
;;

;; Enabling semantic-mode(emacs30.x) was causing  c/c++ mode to slowly
;; import files to  emacs and badly hanging(can't do  any coding) when
;; you   start  editing   the   file,  untill   a   long  timeout   is
;; reached(timeout of the order of 30sec).  CEDET/semantic-mode is now
;; Legacy  and known  not to  work  properly with  modern versions  of
;; emacs.  Don't use semantic-mode for c++ anymore-use LSP.
...
;; ;; 	"/usr/include/c++/11/bits/this_thread_sleep.h"
;; ;;         ))
;; (setq semantic-idle-scheduler-idle-time 0.5) ; seconds
...
;;   (add-hook 'c-mode-hook 'irony-mode)
;;   ;;
;;   ;; maybe needs these two lines to work for me, as it stopped working
;;   ;; at some point
;;   ;;
;;   (add-hook 'simones-c++-mode-hook 'irony-mode)
;;   (add-hook 'simones-c-mode-hook 'irony-mode)
...
(with-eval-after-load 'company
  ...
  ;; at some point
  ;;
  (add-hook 'simones-c++-mode-hook 'company-mode)
  (add-hook 'simones-c-mode-hook 'company-mode)
  )
;; END:
...
;; Sets company mode to only search current buffers and now allows you
;; to  edit text  files with  company-mode completion.   It fixes  the
;; error:
;; Company: An  error occurred  in auto-begin  Company: backend
;; company-dabbrev  error  "Wrong  type  argument:  stringp,  #<buffer
;; quadcopter.cpp>"    with    args    (candidates   and    )    (setq
;;
...
;; The c/c++ compiler knows where the include files are, so your code
;; builds.  But unless you give flycheck the include paths it will
;; flag an error. Use the following commmand to see what your include
;; paths are: *** g++ -E -x c++ - -v < /dev/null ***. To keep using
;; clang you must tell flycheck where the libstdc++ lives.

;; In  your  ~/.clangd enter  the  following  as  well, not  sure  its
;; necessary if you have the include path here.
;;
;;CompileFlags:
...
(use-package flycheck ;; defer loading of package untill you use it.
  ...
  ;; flycheck is using.  I have c/c++-glang.
  (setq flycheck-c/c++-clang-executable t)
...
;; THIS conflicts with treemacs add project via menu ;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
...
;; helm-gtags for python mode, all commands don't seem to work, like C-c g g
;; use ctags instead for python mode.  You need to follow:
;;https://blade6570.github.io/soumyatripathy/blog_gnuglobal/gnu_global.html
;;https://www.spacemacs.org/layers/+tags/gtags/README.html
;;    (add-hook 'python-mode-hook 'helm-gtags-mode)
;; ie:
;; sudo apt-get install global
;; cp gtags.conf ~/.globalrc
;; setenv GTAGSLABEL pygments, in .login
;; MAY also need:
;; exuberant ctags
...
;;(provide 'setup-arduino-company)



;; disable minor mode labels from modeline
;;
